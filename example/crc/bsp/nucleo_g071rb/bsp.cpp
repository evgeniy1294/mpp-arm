...
bool bsp::TestSequenceCheck()
{
  ...
  // Part test
  HardwareLogic::Calculate(CRC, TestSequence.data() , 5);
  HardwareLogic::Calculate(CRC, TestSequence.data() + 5, TestSequence.size() - 5);
  crc = HardwareLogic::Finalize(CRC);
  ...
  // Unaligned data test
  HardwareLogic::Configure< POSIX_32 >(CRC);
  HardwareLogic::Calculate(CRC, TestSequenceUnaligned.data()+2, TestSequenceUnaligned.size()-2);
  crc = HardwareLogic::Finalize(CRC);
